...
public class GlowneOkno extends JFrame {
    ...
    private void zaladujDanePrzykladowe() {
        ...
        Lekarz l3 = new Lekarz("Piotr",   "Wojcik",      "75030312347", LocalDate.of(1975,  3, 22), "L003", "Ortopedia",    "PWZ100003");
    ...
    private void inicjalizujKomponenty() {
        ...
        tabbedPane.addTab("Pacjenci", zbudujPanelPacjentow());
        tabbedPane.addTab("Wizyty",   zbudujPanelWizyt());
        tabbedPane.addTab("Personel", zbudujPanelPersonelu());
        tabbedPane.addTab("Kolejka",  zbudujPanelKolejki());
        tabbedPane.addTab("Oddzialy", zbudujPanelOddzialow());

        // Odswiez combo przy przelaczaniu zakladek
        tabbedPane.addChangeListener(e -> {
            int idx = tabbedPane.getSelectedIndex();
            if (idx == 1) odswiezKomboWizyt();
        });
    ...
    private JPanel zbudujPanelPacjentow() {
        ...
        tfChoroby.setToolTipText("Oddziel przecinkiem");
        ...
        btnDodaj.addActionListener(e -> {
            ...
            try {
                ...
                for (String a : tfAlergie.getText().trim().split(",")) {
                    if (!a.trim().isEmpty()) nowy.dodajAlergie(a.trim());
                }
                for (String ch : tfChoroby.getText().trim().split(",")) {
                    if (!ch.trim().isEmpty()) nowy.dodajChorobe(ch.trim());
                }
        ...
        btnEksport.addActionListener(e -> {
            ...
            Pacjent p = system.wyszukajPacjenta((String) modelTabeliPacjentow.getValueAt(row, 3));
            if (p != null) {
                String sciezka = "karta_" + p.pobierzPesel() + ".txt";
        ...
        JPanel dolne = new JPanel(new FlowLayout(FlowLayout.LEFT));
        dolne.add(btnKarta); dolne.add(btnKolejka); dolne.add(btnSzukaj); dolne.add(btnEksport);

        panel.add(new JScrollPane(tabela), BorderLayout.CENTER);
        panel.add(formularz, BorderLayout.NORTH);
        panel.add(dolne, BorderLayout.SOUTH);
        return panel;
    }
    ...
    private JPanel zbudujPanelWizyt() {
        ...
        btnUmow.addActionListener(e -> {
            ...
            try {
                LocalDateTime data = LocalDateTime.parse(tfData.getText().trim(),
        ...
        btnZmienStatus.addActionListener(e -> {
            ...
            String wybrany = (String) JOptionPane.showInputDialog(this,
                    "Wybierz akcje (status: " + wizyta.pobierzStatus() + "):",
            ...
            switch (wybrany) {
                ...
                case "Zakonczona (wymaga diagnozy)" ->
                    JOptionPane.showMessageDialog(this, "Uzyj przycisku 'Wystaw diagnoze'.", "Informacja", JOptionPane.INFORMATION_MESSAGE);
                case "Odwolana" -> {
                    String powod = JOptionPane.showInputDialog(this, "Podaj powod odwolania:");
                    if (powod != null) {
                        wizyta.anulujWizyte(powod.isBlank() ? "Brak podanego powodu" : powod);
                        odswiezTabeleWizyt();
                    }
                }
            }
        ...
        btnDiagnoza.addActionListener(e -> {
            ...
            // Lekarz jest bezposrednio dostepny z obiektu wizyty
            Lekarz lekarzWizyty = wizyta.pobierzLekarza();

            JTextField tfOpis     = new JTextField(25);
            JTextField tfKodICD   = new JTextField(10);
            JTextField tfZalecenia = new JTextField(25);
            ...
            if (JOptionPane.showConfirmDialog(this, dp, "Wystaw diagnoze",
                    JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION) {
                if (tfOpis.getText().trim().isEmpty() || tfKodICD.getText().trim().isEmpty()) {
                    JOptionPane.showMessageDialog(this, "Opis i kod ICD sa wymagane.", "Blad", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                lekarzWizyty.wystawDiagnoze(wizyta,
                        tfOpis.getText().trim(), tfKodICD.getText().trim(), tfZalecenia.getText().trim());
                odswiezTabeleWizyt();
                odswiezTabelePacjentow();
                JOptionPane.showMessageDialog(this, "Diagnoza wystawiona. Wizyta zakonczona.", "Sukces", JOptionPane.INFORMATION_MESSAGE);
            }
        });

        JPanel dolne = new JPanel(new FlowLayout(FlowLayout.LEFT));
        dolne.add(btnSzczegoly); dolne.add(btnZmienStatus); dolne.add(btnDiagnoza);
    ...
    private void odswiezKomboWizyt() {
        ...
        for (Pracownik pr : system.pobierzListePracownikow())
            if (pr instanceof Lekarz l) cbLekarzWizyt.addItem(l);
    }
    ...
    private JPanel zbudujPanelPersonelu() {
        ...
        JTextField tfImie = new JTextField(10), tfNazwisko = new JTextField(10),
                   tfPesel = new JTextField(11), tfData = new JTextField(10),
                   tfSpec  = new JTextField(10), tfPwz  = new JTextField(10),
        ...
        btnDodajLekarza.addActionListener(e -> {
            try {
                Lekarz lekarz = new Lekarz(
                        tfImie.getText().trim(), tfNazwisko.getText().trim(),
    ...
    private JPanel zbudujPanelKolejki() {
        ...
        JButton btnOdswierz = new JButton("Odswiez");
    ...
    private JPanel zbudujPanelOddzialow() {
        ...
        btnDodajOddzial.addActionListener(e -> {
            try {
                Oddzial o = new Oddzial(tfNazwa.getText().trim(), tfNumer.getText().trim(),
                        Integer.parseInt(tfLozka.getText().trim()));
                system.dodajOddzial(o);
                odswiezTabeleOddzialow();
        ...
        // Aktualizuje etykiety przyciskow w zaleznosci od zaznaczonego oddzialu i wybranego elementu
        Runnable odswiezPrzyciski = () -> {
            ...
            btnPracownik.setText(wybrPr != null && od.pobierzPracownikow().contains(wybrPr)
                    ? "Usun pracownika z oddzialu" : "Dodaj pracownika do oddzialu");

            Pacjent wybrPac = (Pacjent) cbPacjenci.getSelectedItem();
            btnPacjent.setText(wybrPac != null && od.pobierzPacjentow().contains(wybrPac)
                    ? "Wypisz pacjenta z oddzialu" : "Przyjmij pacjenta na oddzial");
        };

        tabela.getSelectionModel().addListSelectionListener(ev -> { if (!ev.getValueIsAdjusting()) odswiezPrzyciski.run(); });
        cbPracownicy.addActionListener(ev -> odswiezPrzyciski.run());
        cbPacjenci.addActionListener(ev -> odswiezPrzyciski.run());
        ...
        btnPracownik.addActionListener(e -> {
            ...
            if (od.pobierzPracownikow().contains(wybrany)) {
                ...
                JOptionPane.showMessageDialog(this, "Usunieto: " + wybrany + " z " + od.pobierzNazwe(), "Sukces", JOptionPane.INFORMATION_MESSAGE);
            } else {
                od.dodajPracownika(wybrany);
                JOptionPane.showMessageDialog(this, "Dodano: " + wybrany + " do " + od.pobierzNazwe(), "Sukces", JOptionPane.INFORMATION_MESSAGE);
            }
        ...
        btnPacjent.addActionListener(e -> {
            ...
            if (od.pobierzPacjentow().contains(wybrany)) {
                ...
                JOptionPane.showMessageDialog(this, "Wypisano: " + wybrany + " z " + od.pobierzNazwe(), "Sukces", JOptionPane.INFORMATION_MESSAGE);
            } else {
                if (!od.przyjmijPacjenta(wybrany))
                    ...
                    JOptionPane.showMessageDialog(this, "Przyjeto: " + wybrany + " na " + od.pobierzNazwe(), "Sukces", JOptionPane.INFORMATION_MESSAGE);
            }
            odswiezTabeleOddzialow(); odswiezPrzyciski.run();
        });
        ...
        btnStatus.addActionListener(e -> {
            ...
            StringBuilder sb = new StringBuilder(od.wyswietlStatus());
            sb.append("\n\n").append(od.wyswietlPracownikow());
            sb.append("\n\nPacjenci na oddziale (").append(od.pobierzPacjentow().size()).append("):\n");
            if (od.pobierzPacjentow().isEmpty()) {
                sb.append("Brak pacjentow.\n");
            } else {
                int i = 1;
                for (Pacjent p : od.pobierzPacjentow()) {
                    sb.append(i++).append(". ")
                      ...
                      .append("  |  Gr. krwi: ").append(p.pobierzGrupeKrwi()).append("\n");
                }
            }
        ...
        // Odswiez combo przy wejsciu na zakladke Oddzialy
        tabbedPane.addChangeListener(e -> {
    ...
    private void odswiezTabelePacjentow() {
        ...
        for (Pacjent p : system.pobierzListePacjentow()) {
            modelTabeliPacjentow.addRow(new Object[]{
                    ...
                    p.pobierzPesel(), p.pobierzGrupeKrwi(), p.czyWPoczekalni() ? "Tak" : "Nie"
            });
        }
    }
    ...
    private void odswiezTabeleWizyt() {
        ...
        for (Wizyta w : system.pobierzListeWizyt()) {
            modelTabeliWizyt.addRow(new Object[]{
                    ...
                    w.pobierzLekarza().toString(), w.pobierzDate().format(fmt),
    ...
    private void odswiezTabelePersonelu() {
        ...
        for (Pracownik p : system.pobierzListePracownikow()) {
            String typ = switch (p) {
                case Lekarz ignored       -> "Lekarz";
                case Pielegniarka ignored -> "Pielęgniarka";
                case Recepcjonista ignored -> "Recepcjonista";
                default -> "Pracownik";
            };
            modelTabeliPersonelu.addRow(new Object[]{
                    p.pobierzIdPracownika(),
                    p.pobierzImie() + " " + p.pobierzNazwisko(),
                    typ,
                    p.pobierzSpecjalizacje(),
                    p.czyAktywny() ? "Tak" : "Nie"
            });
        }
    }
    ...
    private void odswiezTabeleOddzialow() {
        ...
        for (Oddzial o : system.pobierzListeOddzialow()) {
            modelTabeliOddzialow.addRow(new Object[]{
                    o.pobierzNazwe(), o.pobierzNumer(), o.pobierzMaxLozek(),
                    o.dostepneLozka(), o.pobierzPacjentow().size()
            });
        }
    }
